	...
	if(args.isotope == "Bi214" or args.isotope == "Po214"):
		...
		for iConst1 in range(len(parameters)):
			...
			try:
				...
				print(f"{path}/residuals/{args.iteration}/{isotope}/{macName}.npy")
			...
			#print(( Norm_data_histcounts[:100]))
			a = np.sqrt(( Norm_data_histcounts[:MCindex-1] - Norm_MC_histcounts[:MCindex-1] )**2)
			b = np.sqrt((Norm_MC_histcounts_err[:MCindex-1]**2 + Norm_Data_histcounts_err[:MCindex-1]**2  ))
			print(np.sum(Norm_data_histcounts[:MCindex-1]))
			print(np.sum(Norm_MC_histcounts[:MCindex-1]))
			print(np.sum(a/b))
